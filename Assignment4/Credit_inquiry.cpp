...
enum class RequestType {ZERO_BALANCE = 1, CREDIT_BALANCE, DEBIT_BALANCE, END};

RequestType getRequest();
bool shouldDisplay(RequestType type, double);
    ...
    while (request != RequestType::END){
        switch (request){
            case RequestType::ZERO_BALANCE: cout << "\nAccounts with zero balances: \n";
                break;
            case RequestType::CREDIT_BALANCE: cout << "\nAccounts with credit balances: \n";
                break;
            case RequestType::DEBIT_BALANCE: cout << "\nAccounts with debit balances: \n";
                break;
        }
...
//This is safer and shorter than using eof()
        inClientFile.clear(); //reset eof for next input 
        inClientFile.seekg(0); //reposition to the beginning of the file
        request = getRequest(); //get an additional request from user
    }

    cout << "End of run." << endl;
}

RequestType getRequest(){ // obtain request from the user
    ...
    int type; //request from the user
